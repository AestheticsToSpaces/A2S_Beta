...
from config import GEMINI_API_KEY, GEMINI_MODEL, GROQ_API_KEY, GROQ_MODEL, TEMPERATURE, TOP_P, MAX_RESULTS_PER_QUERY
...
try:
    if not GEMINI_API_KEY or GEMINI_API_KEY == "PLACEHOLDER_API_KEY":
        print("[LLM] GEMINI_API_KEY is missing or placeholder. Running in Mock Mode.")
        _client = None
    else:
        _client = genai.Client(api_key=GEMINI_API_KEY)
except Exception as e:
    print(f"[LLM] Failed to initialize Gemini client: {e}. Running in Mock Mode.")
    _client = None
...
try:
    if not GROQ_API_KEY:
        print("[LLM] GROQ_API_KEY is missing. Groq Reasoning will fallback to Mock Mode.")
        _groq_client = None
    else:
        from groq import Groq
        _groq_client = Groq(api_key=GROQ_API_KEY)
except Exception as e:
    print(f"[LLM] Failed to initialize Groq client: {e}. Running in Mock Mode.")
    _groq_client = None
...
def process_message(
    ...
    try:
        ...
        if _client is None:
            # Mock behavior
            response_text = f"I'm using Groq for reasoning, but my initial JSON extraction (Gemini) is in Mock Mode. Let's find some products!"
            ...
            try:
                ...
                if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                    print(f"[LLM] Initial Gemini call rate limited. Proceeding with fallback extraction.")
                    response_text = "I'm currently falling back to Groq due to rate limits on my primary model, but I can still help you find what you need!\n\n"
                    detected_type = _detect_product_type_from_message(user_message)
                    parsed = {
                        "response_text": response_text,
        ...
        if should_show:
            ...
            if not search_results.empty:
                ...
                if len(products_list) > 0 and _groq_client is not None:
                    try:
                        product_details = ""
                        ...
                        reasoning_response = _groq_client.chat.completions.create(
                            messages=[
                                {
                                    "role": "system",
                    ...
                    response_text += "\n\n**[Mock Mode] Editorial Ranking** (Groq API Key Missing):\n"
